...
resource "aws_instance" "bastion" {
  ...
  user_data = <<-EOF
    ...
    # Create a script that will find and use the instance's own ID
    cat > /home/ec2-user/find_instance_id.sh << 'FIND_ID'
    #!/bin/bash
    # Get this instance's ID
    INSTANCE_ID=$(curl -s http://169.254.169.254/latest/meta-data/instance-id)
    echo "This bastion's instance ID is: $INSTANCE_ID"
    echo "Use this ID in your SSH commands:"
    echo "aws ssm start-session --target $INSTANCE_ID --document-name AWS-StartPortForwardingSession --parameters \"localPortNumber=8022,portNumber=22\""
    FIND_ID
    
    chmod +x /home/ec2-user/find_instance_id.sh
    chown ec2-user:ec2-user /home/ec2-user/find_instance_id.sh
...
# Add an additional output to get the instance ID after creation
...
output "bastion_ssm_session_command" {
  description = "Command to start an SSM port forwarding session to the bastion"
  value       = "aws ssm start-session --target ${aws_instance.bastion.id} --document-name AWS-StartPortForwardingSession --parameters \"localPortNumber=8022,portNumber=22\""
}
