# Consolidated Outputs for Clixx Retail Infrastructure
...
output "high_availability_status" {
  ...
  value = {
    ...
    primary_asg       = aws_autoscaling_group.clixx_asg.name
    primary_asg_subnet = aws_subnet.private_app[0].id
    secondary_asg     = aws_autoscaling_group.clixx_asg_secondary.name
    secondary_asg_subnet = aws_subnet.private_app[1].id
...
# Primary ASG outputs (not duplicated elsewhere)
...
output "primary_asg_az" {
  description = "Availability zone for the primary ASG"
  value       = var.availability_zones[0]
}

# Additional certificate outputs (not duplicated elsewhere)
...
output "website_url" {
  ...
  value       = var.create_dns_record ? "https://${var.hosted_zone_record_name}" : "https://${aws_lb.clixx_alb.dns_name}"
}

# Health check URL
output "alb_health_check_path" {
  description = "Path used for ALB health checks"
  value       = aws_lb_target_group.clixx_tg.health_check[0].path
}
